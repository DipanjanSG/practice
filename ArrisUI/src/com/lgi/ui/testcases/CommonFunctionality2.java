...
//		Driver.openModel();
	...
	public String MercuryLogin() {
		...
		int count =0;
		if (!response.equals("Pass") && count<=2)
		{
			response=this.MercuryInitialSetup();
			closeAndOpenDriver();
			response=this.MercuryLogin();
			count++;
		}
	...
	public String disableGuestNetwork() {
	...
	public int DownstreamFrequency(int noOfCh, int ch_csv) {
		...
		for (i = 1; i <= noOfCh; i++) {
			...
			chId = Integer.parseInt(downstreamChFreq);
	...
	public int UpstreamFrequency(int noOfCh, int ch_csv) {
